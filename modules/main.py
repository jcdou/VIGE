...
# Mengimpor modul config secara keseluruhan untuk menghindari circular import dengan settings
...
def check_required_assets():
    ...
    media_dir_path = os.path.join(project_root, config_module.CONFIG.get('MEDIA_DIR', 'assets/media'))
    ...
    music_dir_path = os.path.join(project_root, config_module.CONFIG.get('MUSIC_DIR', 'assets/music'))
    ...
    caption_json_filename = os.path.basename(config_module.CONFIG.get('CAPTION_JSON', 'caption.json'))
    caption_txt_filename = os.path.basename(config_module.CONFIG.get('CAPTION_TXT', 'caption.txt'))
...
def generate_video_process():
    ...
    media_full_path = os.path.join(project_root, config_module.CONFIG['MEDIA_DIR'])
    available_videos = renderer._show_numbered_files(media_full_path, (".mp4", ".avi", ".mov", ".mkv"))  
    
    if not available_videos:  
        print(f"{Fore.YELLOW}Tidak ada file video ditemukan di {config_module.CONFIG['MEDIA_DIR']}.{Style.RESET_ALL}")
        input(f"{Fore.CYAN}Tekan ENTER untuk kembali ke menu utama...{Style.RESET_ALL}")  
        return  
    ...
    music_full_path = os.path.join(project_root, config_module.CONFIG['MUSIC_DIR'])
    available_audio = renderer._show_numbered_files(music_full_path, (".mp3", ".wav", ".aac"))  
    
    if not available_audio:  
        print(f"{Fore.YELLOW}Tidak ada file audio ditemukan di {config_module.CONFIG['MUSIC_DIR']}.{Style.RESET_ALL}")
        input(f"{Fore.CYAN}Tekan ENTER untuk kembali ke menu sebelumnya...{Style.RESET_ALL}")  
        return  
...
def batch_generate_videos_process():
    ...
    if not all_videos_paths:
        print(f"{Fore.YELLOW}Tidak ada file video tersedia untuk batch generation di {config_module.CONFIG['MEDIA_DIR']}.{Style.RESET_ALL}")
        input(f"{Fore.CYAN}Tekan ENTER untuk kembali ke menu utama...{Style.RESET_ALL}")
        return
    if not all_audio_paths:
        print(f"{Fore.YELLOW}Tidak ada file audio tersedia untuk batch generation di {config_module.CONFIG['MUSIC_DIR']}.{Style.RESET_ALL}")
        input(f"{Fore.CYAN}Tekan ENTER untuk kembali ke menu utama...{Style.RESET_ALL}")
        return
...
def reset_overlay_files():
    ...
    files_to_delete = [
        os.path.join(overlay_base_path, os.path.basename(config_module.CONFIG.get('CAPTION_JSON', 'caption.json'))),
        os.path.join(overlay_base_path, os.path.basename(config_module.CONFIG.get('CAPTION_TXT', 'caption.txt'))),
        os.path.join(overlay_base_path, os.path.basename(config_module.CONFIG.get('TREND_CACHE_FILE', 'trend_cache.json'))),
        os.path.join(overlay_base_path, os.path.basename(config_module.CONFIG.get('TREND_TXT', 'trend.txt')))
    ]
...
def trend_and_caption_menu():
    while True:
        ...
        if choice == '1':  
            ...
            print(" 9. millenial")
            print("10. kesehatan")
            print("11. Mix (Acak dari semua gaya)")
            ...
            style_choice = input("Pilih nomor tema (1-11, default 4): ").strip()
            style_name = style_options.get(style_choice, "nalar")
            
            style_name = style_mapping.get(style_choice_num, 'nalar')  
            
            # --- Tambahan: Input dinamis untuk TREND_LIMIT ---
            # Menggunakan config_module.CONFIG untuk mendapatkan nilai default
            num_trends_input = input(f"{Fore.CYAN}Masukkan jumlah tren yang akan diambil (default: {config_module.CONFIG.get('TREND_LIMIT', 5)}, kosongkan untuk default): {Style.RESET_ALL}").strip()
            num_trends_to_fetch = None
            if num_trends_input:
                try:
                    num_trends_to_fetch = int(num_trends_input)
                    if num_trends_to_fetch <= 0:
                        print(f"{Fore.RED}Jumlah tren harus lebih besar dari 0. Menggunakan default.{Style.RESET_ALL}")
                        num_trends_to_fetch = None # Kembali ke default jika input tidak valid
                except ValueError:
                    print(f"{Fore.RED}Masukan tidak valid. Menggunakan default.{Style.RESET_ALL}")
                    num_trends_to_fetch = None # Kembali ke default jika input tidak valid
            # --- Akhir Tambahan ---

            try:  
                # Meneruskan num_trends_to_fetch ke update_trends
                generated_captions = update_trends(style_name, dynamic_trend_limit=num_trends_to_fetch)  
